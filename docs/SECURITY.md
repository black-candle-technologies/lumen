...
Milestone 3 privileged Linux plugin-sandbox verification passed in GitHub Actions run `29381403861` using the repository Linux sandbox workflow. The local macOS suite still proves the macOS sandbox path and platform-independent Linux profile construction tests, but Linux release confidence comes from that privileged CI gate.
