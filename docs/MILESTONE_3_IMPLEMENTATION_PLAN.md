...
Lean Linux gate command: `lumen-integrations` now supports a sandbox-only build with `--no-default-features`, leaving model-client, native-secret, and WASM-host dependencies out of the privileged Linux check. On a healthy Linux target, build `crates/lumen-extension-sdk/examples/subprocess_tool`, then run `cargo test -p lumen-integrations --no-default-features --lib sandbox::tests`, `cargo test -p lumen-integrations --no-default-features --test extension_process -- --nocapture`, and `cargo test -p lumen-integrations --no-default-features --test local_executors system_sandbox -- --nocapture`.
