...
At parent `a5352cbcfe85ce56ac9b4be89f4ec3651c2fb143`, Windows Node 24.19.0 and pnpm 12.4.2 initially loaded both Windows bindings. WSL Node 24.19.0 and pnpm 12.4.1 in that same tree failed `vite build` with missing `@rolldown/binding-linux-x64-gnu`, and `check:native` named both missing Linux packages.

At implementation commit `087edeb`, separate frozen installs each resolved 151 packages without changing either lockfile (SHA-256 `2f8155606aad6bafa323dc120f6326e3fae7c9ac28c7acbfaaf8e1cf9cbb8679`). On Windows, preflight, Svelte check (0 errors, 0 warnings), Vitest (21/21), Vite build, Tauri CLI 2.11.2, and default Vite dev (`http://localhost:5173/`, HTTP 200, `::1` listener only) passed. In the detached WSL checkout, preflight and its two regression tests, Svelte check (0 errors, 0 warnings), Vitest (21/21), Vite build, Tauri CLI 2.11.2, and default Vite dev (ready in 18.9 seconds after re-optimization, HTTP 200, `127.0.0.1` listener only) passed. Both dev processes were stopped and port 5173 closed on each host. Windows preflight remained green after WSL installation. No native desktop build or live application run was claimed.
