...
impl Database {
    pub async fn reserve_worker_attempt(
        ...
        if let Some(route) = sqlx::query("SELECT decision_id,reservation_id FROM routing_budget_reservations WHERE orchestration_id=? AND graph_revision=? AND task_node_id=? AND state='active' ORDER BY created_at DESC LIMIT 1")
            .bind(assignment.orchestration_id().to_string()).bind(pos(assignment.graph_revision())?).bind(assignment.task_node_id().to_string()).fetch_optional(&mut *tx).await? {
            sqlx::query("INSERT INTO worker_attempt_routing_bindings(attempt_id,decision_id,reservation_id,created_at) VALUES(?,?,?,?)")
                .bind(attempt_id.to_string()).bind(route.try_get::<String, _>("decision_id")?).bind(route.try_get::<String, _>("reservation_id")?).bind(stamp).execute(&mut *tx).await?;
        }
        sqlx::query("UPDATE worker_retry_decisions SET new_attempt_id=? WHERE decision_id=(SELECT decision_id FROM worker_retry_decisions WHERE orchestration_id=? AND graph_revision=? AND task_node_id=? AND allowed=1 AND new_attempt_id IS NULL ORDER BY created_at DESC,rowid DESC LIMIT 1)")
            .bind(attempt_id.to_string()).bind(assignment.orchestration_id().to_string()).bind(pos(assignment.graph_revision())?).bind(assignment.task_node_id().to_string()).execute(&mut *tx).await?;
        tx.commit().await?;
